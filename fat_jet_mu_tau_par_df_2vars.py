...
parser.add_argument("--quantity1", help="The first distribution you want to plot", type=str)
parser.add_argument("--quantity2", help="The second distribution you want to plot", type=str)
...
data_dict = df.AsNumpy([args.quantity1, args.quantity2,  "fj_Xtm_particleNet_XtmVsQCD", "eles_finalstate", "mu_tau_finalstate",
                         "fj_XtmVsQCD_pt", "fj_Xtm_msoftdrop", "fj_Xtm_muon_subj_deltaR", "fj_Xtm_muon_subj_deltaPt"])
pandas_df = pd.DataFrame(data_dict)


legend_inf = get_legend_info(args.quantity1)
...
df_pnet = pandas_df[(pandas_df["fj_Xtm_particleNet_XtmVsQCD"] > args.pnetcut) & (pandas_df[args.quantity1] != unphys_value) & finalstate & fatjet_pt_cut]
...
plt.xlabel(label)
plt.ylabel("dN")
plt.hist(df_pnet[args.quantity1].values, args.bins, label=r"$p_{T}$ of subjet in vicinity of muon" )
plt.hist(df_pnet[args.quantity2].values, args.bins, label=r"$p_{T}$ of muon in fatjet" )
...
plt.savefig(args.quantity1+"_"+args.quantity2+"_zp_incl_pnet_{cut}.pdf".format(cut=str(args.pnetcut)))